...
class ImageCapture {
    ...
    // 加载文件
    async mountFile(file: File | Blob |any, MOUNT_DIR: string, id: number) {
        if (!this.isMKDIR) {
            FS.mkdir(MOUNT_DIR);
            this.isMKDIR = true;
        }
        
        let data: {files?: File[], blobs?: Array<{name: string, data: Blob}>} = {};
        if(typeof file === "string"){
            file=await this.getFileBuffer(file)
        }
        let name: string = '';
        // 判断类型 如果是blob转file
    ...
    async capture({id, info, path = '/working', file}) {
        try {
            const name = await this.mountFile(file, path, id);
            let retData = 0;
            this.imageList[id] = [];

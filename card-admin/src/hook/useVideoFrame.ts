...
import * as cheetahCapture from "./capture/index";
import { createFFmpeg, fetchFile, createFfm } from "@ffmpeg/ffmpeg";
...
export const useVideoFrame = () => {
  ...
  const workerPath = new URL(location.origin + "/capture/capture.worker.js");
  const wasmPath = new URL(location.origin + "/capture/capture.worker.wasm");
